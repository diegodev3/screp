...
(function() {
  ...
  /*
  Rodrigo Mendonça, Diego e Inácio
   */
  this.moedaContabil = function(val) {
    if (val < 0) {
      return "(" + (dinheiro(Math.abs(val))) + ")";
    } else {
      return dinheiro(val);
    }
  };

  this.decimal = function(val) {
    return dinheiro(val, {
      unidade: ''
    });
  };

  this.porcentagem = function(val) {
    return "" + (dinheiro(val, {
      unidade: ''
    })) + "%";
  };

  this.dinheiro_bd = function(money, symbol) {
    var _ref;
    if (symbol == null) {
      symbol = 'R$';
    }
    ...
    if ((_ref = typeof money) === 'string' || _ref === 'object') {
      if (/,/.test(money)) {
        money = money.replace(/\./g, '').replace(',', '.');
      }
      money = parseFloat(money.replace(/\s+/g, '').replace(symbol, ''));
    }
    return money;
  };

  this.dinheiro = function(num, options) {
    var cents, dotPosition, i, isNegative, opt, ret;
    num = dinheiro_bd(num);
    opt = {
    ...
    while (i < Math.floor((num.length - (1 + i)) / 3)) {
      dotPosition = num.length - (4 * i + 3);
      num = "" + (num.slice(0, dotPosition)) + "." + (num.slice(dotPosition));
      i++;
    }
    ret = "" + num + "," + cents;
...
(function() {
  var getMousePositionFromClickedContainer;
  ...
  this.baseUrlCliente = function(clienteId) {
    var mes_atual, url;
    mes_atual = (typeof currentMonthYear === "function" ? currentMonthYear() : void 0) || scTimezone.now.format('MM-YYYY');
    url = '/gerenciar/cd/' + clienteId + '/' + mes_atual;
    return url;
  };

  this.currentUser = function() {
    var _ref;
    return (_ref = $('body').attr('data-current_user')) != null ? _ref.toJson() : void 0;
  };

  this.currentCliente = function() {
    var k, keys, obj, _i, _len, _ref;
    keys = (_ref = $('body').attr('data-cliente_keys')) != null ? _ref.toJson() : void 0;
    obj = {};
    if (keys) {
      for (_i = 0, _len = keys.length; _i < _len; _i++) {
        k = keys[_i];
        obj[k] = k === 'id' ? parseInt($('body').attr("data-cliente_" + k)) : $('body').attr("data-cliente_" + k);
      }
    }
    return obj;
  };
  ...
  this.currentDominioEPortaServerSocket = function() {
    switch (currentEnviroment()) {
      case "development":
        return "" + document.location.hostname + ":5200";
      default:
  ...
  this.diferencaEntreDatas = function(data_menor, data_maior, interval) {
    ...
    month = day * 30;
    year = month * 12;
    date1 = scTimezone["new"](data_menor);
    date2 = scTimezone["new"](data_maior);
  ...
  this.lastDayMonth = function(date) {
    var ax, initD, lastDay;
    ax = date.split('-');
    initD = new Date(ax[0], ax[1] - 1, ax[2]);
    lastDay = new Date(initD.getFullYear(), initD.getMonth() + 1, 0);
...
(function() {
  this.scPageSystem = {
    ...
    updatePageSettingsFromServer: function(url, onSuccess) {
      var paramsUrl;
      enableBodyLoading();
      showBodyLoading();
      ...
      return scPageSystem.reqGetBodySettings = $.ajax({
        ...
        error: function(XMLHttpRequest, textStatus, errorThrown) {
          hideBodyLoading();
          if (textStatus !== "abort") {
            return alert("Desculpe erro inesperado ao carregar seu fuso horário");
          }
        }
      });
    },
    ...
    timezone: {
      "new": function() {
        var ano, bits, dia, is_DD_MM_YYYY_format, is_YYYY_MM_DD_format, mes, regexDD_MM_YYYY, regexYYYY_MM_DD, __newDateArgs;
        ...
        if (arguments.length === 0) {
          ...
          if (arguments.length === 1) {
            ...
            is_YYYY_MM_DD_format = arguments[0].match(regexYYYY_MM_DD);
            regexDD_MM_YYYY = /^([0-9]|0[1-9]|[1-2][0-9]|3[0-1])\/([1-9]|0[1-9]|1[0-2])\/[0-9]{4}$/;
            is_DD_MM_YYYY_format = arguments[0].match(regexDD_MM_YYYY);
            if (is_YYYY_MM_DD_format || is_DD_MM_YYYY_format) {
              if (is_YYYY_MM_DD_format) {
                bits = arguments[0].split('-');
                ano = bits[0].to_i();
                mes = bits[1].to_i();
                dia = bits[2].to_i();
              }
...
(function() {
  ...
  this.Array.prototype.find = function(value, field) {
  ...
  this.Array.prototype.flatten = function() {
    var b, self, _ref;
    self = this.clone();
    while (self.length !== (b != null ? b.length : void 0)) {
      b = self;
      self = (_ref = []).concat.apply(_ref, self);
    }
    return self;
  };
...
(function() {
  this.Date.prototype.setEndOfDay = function() {
    this.setHours(23);
    this.setMinutes(59);
    this.setSeconds(59);
    return this;
  ...
  Date.getDaysInMonth = function(year, month) {
    return [31, (ano_bissexto(year) ? 29 : 28), 31, 30, 31, 30, 31, 31, 30, 31, 30, 31][month];
  };
...
(function() {
  ...
  this.String.prototype.humanize = function() {
    return this.replace(/^[\s_]+|[\s_]+$/g, '').replace(/[_\s]+/g, ' ').replace(/^[a-z]/, function(m) {
      return m.toUpperCase();
    });
  };

  this.String.prototype.interpolate = function(obj) {
    var k, re, text, v;
    if (!obj) {
      return this;
    }
...
(function() {
  angular.module("sc.commons.app-notifications", ['sc.websocket']).service("$scAppNotifications", [
    '$rootScope', 'Sistema::NotificacaoResource', '$scWebSocket', function($rootScope, Notificacao, ws) {
      var addNotificacao, obj, updateNotificacao;
      obj = {
        pendentes: [],
        nao_lidas: [],
        ...
        buscar: function() {
          return Notificacao.list({
            ...
            return function(data) {
              ...
              _this.items = data.list.items;
              _ref = data.list.items;
              _results = [];
              for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                item = _ref[_i];
      ...
      addNotificacao = function(data) {
        var notificacao, _i, _len, _results;
        _results = [];
        for (_i = 0, _len = data.length; _i < _len; _i++) {
          notificacao = data[_i];
          obj.checkData(notificacao);
          _results.push(obj.items.unshift(notificacao));
        }
        return _results;
      };
      updateNotificacao = function(data) {
        var notificacao, _i, _len, _results;
        _results = [];
        for (_i = 0, _len = data.length; _i < _len; _i++) {
          notificacao = data[_i];
          obj.checkData(notificacao);
          _results.push(obj.items.addOrExtend(notificacao));
        }
        return _results;
      };
      if (currentUser()) {
        ws.scope($rootScope).subscribe("sistema-notificacao-" + (currentUser().id)).on('atualizada', updateNotificacao).on('nova', addNotificacao);
        obj.buscar();
      }
      return obj;
    }
  ...
  angular.module('sc.commons.components.currencyInput', []).directive("scCurrencyInput", function() {
    return {
      ...
      scope: {
        myModel: "=ngModel",
        cb: "=scCallback",
        cbArgs: "=scCallbackArgs"
      },
      link: function($scope, $element, $attrs, ngModelCtrl) {
        var ciCallback, listener;
        ...
        ciCallback = function() {
          ...
          $scope.cbArgs || ($scope.cbArgs = []);
          return $scope.cb.apply($scope, $scope.cbArgs);
        };
        $element.bind("keyup change", function() {
          $scope.$apply(listener);
          return ciCallback();
        });
        return $scope.$watch("myModel", function(newVal, oldVal, scope) {
          if (typeof newVal === "string") {
            $scope.myModel = dinheiro_bd(newVal);
          } else {
            $element.val(dinheiro(newVal || 0));
          }
          return ciCallback();
        });
      }
...
(function() {
  angular.module('sc.commons.components.decimalInput', []).directive("scDecimalInput", function() {
    return {
      require: "ngModel",
      scope: {
        myModel: "=ngModel"
...
(function() {
  angular.module('sc.login', []).service('LoginService', [
    '$resource', '$http', '$location', function($resource, $http, $location) {
      ...
      servico = {
        ...
        gotoSystem: (function(_this) {
          return function() {
            var url;
            _this.leaving = true;
            if ($location.search().save_current_url) {
              url = $location.search().save_current_url;
            } else {
              url = servico.redirect;
            }
            return redirect.url(url);
          };
        })(this),
        callbacksLogin: {
          beforeTry: (function(_this) {
            return function() {
              servico.leaving = false;
              servico.loading = true;
              return servico.hasError = false;
            };
          })(this),
          onSuccess: (function(_this) {
            return function(data) {
              servico.errors = {};
              servico.hasError = false;
              servico.redirect = data.redirect_url;
              servico.me = data;
              servico.gotoSystem();
              return servico.loading = false;
            };
          })(this),
          onFailure: (function(_this) {
            return function(data) {
              servico.redirect = '';
              servico.errors = data.message;
              servico.hasError = true;
              servico.me = void 0;
              return servico.loading = false;
            };
          })(this)
        ...
        withGoogle: function() {
          ...
          return auth2.signIn().then((function(_this) {
            return function(data) {
              var token, url;
              token = data.hg.access_token;
              url = "/auth/google_oauth2_access_token/callback?access_token=" + token;
              _this.callbacksLogin.beforeTry();
              $http.post(url).success(function(data) {
                return _this.callbacksLogin.onSuccess(data);
              }).error(function(data) {
                return _this.callbacksLogin.onFailure(data);
              });
            };
          })(this));
        },
        withFacebook: function(token) {
        ...
        create: function(login) {
          ...
          failureCallback = (function(_this) {
            return function(resp) {
              return _this.callbacksLogin.onFailure(resp.data);
            };
          })(this);
    ...
    '$compile', '$scTimezone', '$document', '$locale', function($compile, $scTimezone, $document, $locale) {
      return {
        ...
        link: function(scope, element, attrs, controllers) {
          ...
          scope.setMonthYear = function(monthYear, init) {
            var monthFullNameString;
            if (init == null) {
              init = false;
            }
            if (scope.ngDisable && !init) {
              return;
            }
          ...
          scope.setMonthYear(yearMonth, true);
...
(function() {
  angular.module('sc.typeahead.queries', []).factory('Typeahead::Queries::Cidade', [
    '$http', function($http) {
      return {
  ...
  ]).factory('Typeahead::Queries::Financeiro::Pessoa', [
    '$http', function($http) {
      return {
        query: function(query, rejects) {
          ...
          return $http({
            method: 'GET',
            responseType: 'json',
            url: "BASEURL/financeiro/pessoas.json",
            params: {
              filtro: {
                q: query
              },
  ...
  ]).factory('Typeahead::Queries::Financeiro::Unidade', [
    '$http', function($http) {
      return {
        query: function(query) {
          return $http({
            method: 'GET',
            responseType: 'json',
            url: "BASEURL/financeiro/unidades.json",
            params: {
              filtro: {
                q: query
              }
  ...
  ]).factory('Typeahead::Queries::Financeiro::Medidor::Unidade', [
    '$http', function($http) {
      return {
        query: function(query) {
          return $http({
            method: 'GET',
            responseType: 'json',
            url: 'BASEURL/financeiro/medidores/buscar_unidade.json',
            params: {
              q: query
            }
          }).then(function(response) {
...
(function() {
  angular.module('sc.commons.components.typeahead', ['sc.commons.factories.position', 'sc.typeahead.queries']).factory('typeaheadParser', [
        ...
        link: function(scope, element, attrs, ngModelCtrl) {
          if (Modernizr.inputtypes.date) {
            ...
            scope.$watch(attrs.ngModel, function(newValue, oldVal, scope) {
              var value;
              value = (newValue != null ? typeof newValue.toISOString === "function" ? newValue.toISOString().split('T')[0] : void 0 : void 0) || newValue;
...
// Como usar
// Hora
// <input sc-mask="99:99" ng-model="xxx" sc-mask-on-complete="funcInnerScope()" />

// Cep
// <input sc-mask="99999-999" ng-model="xxx" sc-mask-on-complete="funcInnerScope()" />

// Data
// <input sc-mask="99/99/99999" ng-model="xxx" sc-mask-on-complete="funcInnerScope()" />

// Letra e número
// <input sc-mask="AA-99" ng-model="xxx" sc-mask-on-complete="funcInnerScope()" />
...
angular.module('sc.commons.directives.mask', [])
  ...
  .directive('scMask', ['uiMaskConfig', '$parse', '$timeout', function (maskConfig, $parse, $timeout) {
    return {
      priority: 100,
      require: 'ngModel',
      restrict: 'A',
...
(function() {
  angular.module('sc.commons.directives.multiCheck', []).directive('multiCheckGroup', function() {
    return {
      scope: {},
      controller: [
        '$scope', function($scope) {
          this.getElements = function() {
            return Array.prototype.slice.call($scope.element[0].querySelectorAll('[multi-check]'), 0);
          };
    ...
    return {
      ...
      link: function(scope, el, attrs, controllers) {
        ...
        return el.bind('click', function(event) {
          var box, checked, chkboxes, end, last, model, start, _i, _len, _ref;
          last = groupCtrl.lastChecked;
          if (last && event.shiftKey) {
            chkboxes = groupCtrl.getElements();
            start = chkboxes.indexOf(event.target);
            end = chkboxes.indexOf(last);
            checked = last.checked;
            _ref = chkboxes.slice(Math.min(start, end), +Math.max(start, end) + 1 || 9e9);
            for (_i = 0, _len = _ref.length; _i < _len; _i++) {
              box = _ref[_i];
              model = angular.element(box).data('$ngModelController');
              model.$setViewValue(checked);
            }
          }
          return groupCtrl.lastChecked = event.target;
        });
      }
    };
  });
...
.directive('pagination', ['$parse', 'paginationConfig', function($parse, paginationConfig) {
  return {
    ...
    template: "<ul class=\"pagination\"><li ng-if=\"boundaryLinks\" ng-class=\"{disabled: noPrevious()}\"><a href ng-click=\"selectPage(1, $event)\">{{getText('first')}}</a></li><li ng-if=\"directionLinks\" ng-class=\"{disabled: noPrevious()}\"><a href ng-click=\"selectPage(page - 1, $event)\">{{getText('previous')}}</a></li><li ng-repeat=\"page in pages track by $index\" ng-class=\"{'active': page.active, 'hidden-xs hidden-sm': !page.active}\"><a href ng-click=\"selectPage(page.number, $event)\">{{page.text}}<span class=\"hidden-md hidden-lg\"> / {{totalPages}}</span></a></li><li ng-if=\"directionLinks\" ng-class=\"{disabled: noNext()}\"><a href ng-click=\"selectPage(page + 1, $event)\">{{getText('next')}}</a></li><li ng-if=\"boundaryLinks\" ng-class=\"{disabled: noNext()}\"><a href ng-click=\"selectPage(totalPages, $event)\">{{getText('last')}}</a></li></ul>",
    replace: true,
    ...
    '$timeout', function($timeout) {
      return {
        link: function(scope, elem, attr) {
          ...
          elem.bind('click', closes);
          return scope.$on('$destroy', function() {
            return elem.unbind('click', closes);
          });
        }
      };
    }
...
/*
  ...
  Como usar:
    1. Importe o serviço $scGallery e passe uma lista de imagens
       para o método open()

      A lista de imagens deve conter uma array de objetos da seguinte forma:
      listaImagens = [
        { img: "URL DA IMAGEM", description: "DESCRIÇÃO OPCIONAL"}
        { img: "URL DA IMAGEM", description: "DESCRIÇÃO OPCIONAL"}
      ]
      $scGallery.open(listaImagens)

    2. Use a directiva "sc-gallery" numa DIV para inserir o HTML da galeria
       no BODY da página no qual será usado
      <div sc-gallery ></div>

    3. Configuracoes
      $scGallery.open(listaImagens, {OPÇÕES})
      -------------------------------------------------------------------------
      |  OPÇÃO  | DEFAULT | DESCRICAO                                         |
      -------------------------------------------------------------------------
      | idx:    | 0       | passe o $index para abrir uma imagem específica   |
      |         |         | na galeria                                        |
      -------------------------------------------------------------------------
      | thumbs: | 'dots'  | use 'dots' para navegar com "bolinhas" ou 'image' |
      |         |         | para mostrar os thumbs, 'none' para desabilitar   |
      -------------------------------------------------------------------------
 */

(function() {
  angular.module('sc.commons.directives.scGallery', ['ngTouch']).service('$scGallery', [
    '$document', '$timeout', '$q', function($document, $timeout, $q) {
      var galleryObj, loadImage, openGallery, showImage, smartScroll, thumb;
      thumb = {
        wrapper: function() {
          return angular.element(document.querySelectorAll('.sc-gallery-thumb-wrapper'));
        },
        thumbnails: function() {
          return angular.element(document.querySelectorAll('.sc-gallery-thumbnails'));
        },
        fullWidth: 0,
        width: 50
      };
      galleryObj = {
        ...
        changeImage: function(i) {
          this.idx = i;
          return showImage(this.idx);
        },
        nextImage: function() {
          this.idx += 1;
          if (this.idx >= galleryObj.list.length) {
            this.idx = 0;
          }
          return showImage(this.idx);
        },
        prevImage: function() {
          this.idx -= 1;
          if (this.idx < 0) {
            this.idx = galleryObj.list.length - 1;
          }
          return showImage(this.idx);
        },
        close: function() {
        ...
        open: function(list, opt) {
          ...
          if (opt.thumbs != null) {
            this.thumbs = opt.thumbs;
          }
          return openGallery(this.idx);
        }
      };
      openGallery = function(idx) {
        if (typeof idx !== void 0) {
          galleryObj.idx = idx;
          showImage(galleryObj.idx);
        }
        galleryObj.opened = true;
        if (galleryObj.thumbs === 'image') {
          $timeout(function() {
            thumb.fullWidth = thumb.width * galleryObj.list.length;
            thumb.thumbnails().css({
              width: "" + thumb.fullWidth + "px"
            });
            return smartScroll(galleryObj.idx);
          }, 100);
        }
      };
      loadImage = function(i) {
        ...
        image.src = galleryObj.list[i].img;
        galleryObj.loading = true;
        return deferred.promise;
      };
      showImage = function(i) {
        loadImage(galleryObj.idx).then(function(resp) {
          galleryObj.img = resp.src;
          smartScroll(galleryObj.idx);
        });
        galleryObj.descr = galleryObj.list[i].description || '';
      };
      smartScroll = function(idx) {
        var position, screenAverage, thumbOfCenter;
        if (galleryObj.thumbs !== 'image') {
          return;
        }
        position = idx + 1;
        screenAverage = thumb.wrapper()[0].offsetWidth / thumb.fullWidth;
        thumbOfCenter = (galleryObj.list.length / 2) * screenAverage;
        return thumb.wrapper()[0].scrollLeft = (position * thumb.width - (thumbOfCenter * thumb.width)) - (thumb.width / 2);
      };
      return galleryObj;
    }
  ]).directive('scGallery', [
    '$document', '$scGallery', function($document, $scGallery) {
      return {
        restrict: 'EA',
        template: '<div class="sc-gallery ng-cloak" ng-cloak ng-swipe-left="scGallery.nextImage()" ng-swipe-right="scGallery.prevImage()" ng-show="scGallery.opened">\n  <div class="sc-gallery-loading" ng-show="scGallery.loading"><i class="sc-icon sc-icon-spin sc-icon-carregando-1"></i></div>\n  <a class="sc-gallery-close" ng-click="scGallery.close()"><i class="sc-icon sc-icon-fechar-1"></i></a>\n  <a class="sc-gallery-nav-left" ng-click="scGallery.prevImage()"><i class="sc-icon sc-icon-seta-7-esquerda"></i></a>\n  <img ng-src="{{ scGallery.img }}" ng-click="scGallery.nextImage()" ng-hide="scGallery.loading" class="{{ scGallery.loading ? \'loading\' : \'\' }}" />\n  <a class="sc-gallery-nav-right" ng-click="scGallery.nextImage()"><i class="sc-icon sc-icon-seta-7-direita"></i></a>\n  <span class="sc-gallery-description">{{ scGallery.descr }}</span>\n  <div class="sc-gallery-thumb-wrapper" ng-hide="scGallery.thumbs == \'none\'">\n    <div class="sc-gallery-thumbnails" ng-if="scGallery.thumbs == \'image\'">\n      <div ng-repeat="i in scGallery.list" style="background-image: url({{ i.img }})" class="sc-thumb" ng-class="{\'active\': scGallery.idx === $index}" ng-click="scGallery.changeImage($index)"></div>\n    </div>\n    <div class="sc-gallery-dots" ng-if="scGallery.thumbs == \'dots\'">\n      <span ng-repeat="i in scGallery.list" class="sc-dot" ng-class="{\'active\': scGallery.idx === $index}" ng-click="scGallery.changeImage($index)"></span>\n    </div>\n  </div>\n</div>',
        link: function(scope, element, attr) {
          var $body, navKeys;
          scope.scGallery = $scGallery;
          $body = $document.find('body');
          navKeys = function(event) {
            var keyCode;
            if (!scope.scGallery.opened) {
              return;
            }
            keyCode = event.which || event.keyCode;
            switch (keyCode) {
              case keyMap.ESC:
                scope.scGallery.close();
                break;
              case keyMap.ARROW.RIGHT:
                scope.scGallery.nextImage();
                break;
              case keyMap.ARROW.LEFT:
                scope.scGallery.prevImage();
            }
            scope.$apply();
          };
...
(function() {
  angular.module('sc.commons.directives.scLoading', []).directive("scLoading", [
    '$timeout', function($timeout) {
      return {
        restrict: "AE",
        scope: {
        ...
        link: function($scope) {
          return $timeout((function() {
            var _ref;
            $scope.message || ($scope.message = 'Carregando');
            return $scope.blockScreen = (_ref = $scope.blockScreen) != null ? _ref : true;
          }), 0);
        },
        template: "<div ng-class=\"{'active': showOn}\" ng-cloak class=\"sc-loading ng-cloak ng-animation-slider\"> <i class=\"sc-icon sc-icon-carregando-1 sc-icon-spin\"></i> {{message}} </div> <div class=\"sc-loading-bg\" ng-class=\"{'active': showOn}\" ng-show=\"showOn\" ng-if=\"blockScreen && showOn\"></div>"
      };
    }
  ]);
  ...
  angular.module('sc.commons.directives.modal', []).directive("scModal", [
    '$scModalService', function($scModalService) {
      return {
        ...
        template: "<div class=\"sc-modal-overlay\" ng-class=\"{'active': modal.active}\" ng-show=\"modal.active\"> <div class=\"sc-modal\" ng-class=\"{'active': modal.active}\" ng-transclude></div> </div>"
      };
    }
      ...
      return {
        link: function(scope, elem, attr) {
          ...
          scope.$on('scScrollOn', function(e, obj, name) {
            var alturaFixa, element, fixedBarsExist, paddingAMais, position, _i, _len, _ref;
            if (obj === object && name === target && $(elem).offset()) {
              alturaFixa = 0;
              _ref = $(".sc-fixed-element");
              for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                element = _ref[_i];
                alturaFixa += $(element).outerHeight();
              }
              fixedBarsExist = typeof fixedBars !== "undefined";
              if (fixedBarsExist) {
                alturaFixa += fixedBars.maxHeight();
              }
              alturaFixa += 60;
...
(function() {
  ...
  this.scScrollTop = function() {
    $("body").animate({
      scrollTop: 0
    }, 300);
  };

  $("body").on('click', '.sc-scroll-top', function() {
    return scScrollTop();
  });
...
(function() {
  angular.module('sc.commons.factories.paginations', []).factory('$scScrollPagination', function() {
    return function() {
      return {
        started_at: void 0,
        page: 0,
        ...
        set: function(other) {
          return angular.extend(this, other);
        },
        reset: function() {
          return this.set({
            started_at: void 0,
...
(function() {
  angular.module("sc.commons.filters.sc.dinheiro", []).filter('dinheiro', function() {
    ...
    return function(val) {
      return moedaContabil(val);
    };
  }).filter('decimal', function() {
    return function(val) {
      return decimal(val);
    };
  }).filter('porcentagem', function() {
    return function(val) {
      return porcentagem(val);
    };
  });
...
(function() {
  ...
  stopScroll = function() {
    var scrollableDist, trapClassName, trapElement, trapSelector, trapWheel;
    trapElement = void 0;
    scrollableDist = void 0;
    trapClassName = 'sc-stop-scroll-enabled';
    trapSelector = '.sc-stop-scroll';
    trapWheel = function(e) {
      var curScrollPos, dY, wheelEvent;
      if (!$('body').hasClass(trapClassName)) {
        ...
        if (dY > 0 && curScrollPos >= (scrollableDist - 2) || dY < 0 && curScrollPos <= 0) {
          return false;
        }
      }
    };
    return $(document).on('wheel', trapWheel).on('mouseleave', trapSelector, function(e) {
      e.stopPropagation();
      $('body').removeClass(trapClassName);
    }).on('mousemove', trapSelector, function(e) {
...
(function() {
  angular.module('sc.app.helpers', ['sc.commons.app-notifications', 'sc.commons.components.typeahead', 'sc.commons.components.currencyInput', 'sc.commons.components.decimalInput', 'sc.commons.components.monthPicker', 'sc.commons.directives.dropdown', 'sc.commons.directives.redirectFocusTo', 'sc.commons.directives.dateInput', 'sc.commons.directives.integerInput', 'sc.commons.directives.timeInput', 'sc.commons.directives.durationInput', 'sc.commons.directives.target', 'sc.commons.directives.sticker', 'sc.commons.directives.scroller', 'sc.commons.directives.scrollTo', 'sc.commons.directives.scLoading', 'sc.commons.directives.scGallery', 'sc.commons.directives.scStopClick', 'sc.commons.directives.modal', 'sc.commons.directives.thumb', 'sc.commons.directives.redactor', 'sc.commons.directives.input.dismiss.click', 'sc.commons.directives.mask', 'sc.commons.directives.multiCheck', 'sc.commons.filters.highlight', 'sc.commons.filters.nl2br', 'sc.commons.filters.sc.time', 'sc.commons.filters.sc.dinheiro', 'sc.commons.filters.sc.queryString', 'sc.commons.filters.showdown', 'sc.commons.filters.i18n', 'sc.commons.filters.string', 'sc.commons.filters.array', 'sc.commons.services.html5support', 'sc.commons.factories.resourceList', 'sc.commons.factories.paginations', 'sc.commons.factories.toggle', 'sc.commons.factories.states', 'sc.commons.factories.icons', 'sc.commons.carousel', 'sc.commons.scTopMessages', 'sc.commons.service.scAlert', 'sc.animations', 'sc.page.settings', 'sc.timezone', 'sc.suporte.queries']);
...
(function() {
  angular.module('sc.commons.service.scAlert', ['sc.commons.factories.toggle']).service('scAlert', [
    ...
    'scToggle', '$filter', '$scTarget', function($toggler, $filter, $scTarget) {
      ...
      data.toggler = new $toggler({
        onOpen: function() {
          ...
          _ref = [this.closed, this.opened].concat(__slice.call(arguments)), data.closed = _ref[0], data.opened = _ref[1], data.list = _ref[2], data.activeIdx = _ref[3], data.module = _ref[4];
      ...
      data.open = function(list, activeIdx, module) {
        var _ref;
      ...
      data.getDownloadUrl = function() {
        var active;
        active = data.list[data.activeIdx];
        return "" + (baseUrlController()) + "/" + data.module + "/anexos/" + active.id + "/download";
      };
      data.getDownloadName = function() {
        return data.list[data.activeIdx].name;
      };
...
(function() {
  angular.module('sc.suporte.queries', []).factory('Suporte::CEP', [
    '$resource', function($resource) {
      return {
        resource: $resource('/suporte/ceps/:numero.json', {
          numero: '@numero'
        }, {
          show: {
            method: 'GET'
        ...
        query: function(numero, sCallback, eCallback) {
          sCallback || (sCallback = angular.noop);
          eCallback || (eCallback = angular.noop);
          if (this.valid(numero)) {
...
(function() {
  ...
  angular.module("sc.commons.scTopMessages", ['sc.commons.factories.toggle']).service("scTopMessages", [
    'scToggle', '$timeout', function($toggler, $timeout) {
      var backgrounds, data, defaultOptions, key, knownThemes, val;
      defaultOptions = {
        theme: 'danger',
        timeOut: void 0,
        delay: 0
      ...
      backgrounds = {
        'warning': 'sc-bg-yellow',
        'success': 'sc-bg-green',
        'danger': 'sc-bg-red',
        'info': 'sc-bg-cian'
      };
      knownThemes = (function() {
        var _results;
        _results = [];
      ...
      data = {
        ...
        open: function(messages, opts) {
          ...
          this.theme = backgrounds[opts.theme];
          $timeout((function(_this) {
            return function() {
